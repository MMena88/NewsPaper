...
def main():
    ...
    # Also respect the FORCE_REPORT env variable set by the GitHub Actions
    # workflow when the user triggers a manual dispatch with force=true.
    force = args.force or os.environ.get("FORCE_REPORT", "false").lower() == "true"
    ...
    # 8:00 AM New York check
    # The workflow runs at 13:00 AND 14:00 UTC to cover both EDT and EST.
    # The check below ensures only the correct run publishes the report.
    if not force:
        if hour != 8:
            print(f"Skipping generation. Current hour is {hour}, report is scheduled for 8:00 AM Eastern Time.")
            print("Use the '--force' flag or set FORCE_REPORT=true to override.")
            sys.exit(0)
        else:
            print("Time check passed. Starting scheduled daily report generation...")
    else:
        print("Force mode active. Proceeding with report generation...")
